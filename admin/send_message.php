...
// CORRECTED: Include the database configuration BEFORE starting the session.
require_once "../db_config.php";
session_start();

// --- SECURITY CHECK: Only admins can send messages ---
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'send_message') {
        ...
        if (empty($subject) || empty($body) || empty($recipient_roles)) {
            echo json_encode(['success' => false, 'message' => 'Subject, message body, and at least one recipient role are required.']);
            exit;
        }
        ...
        try {
            ...
            // 2. Find all users with the selected roles
            ...
            if (empty($recipient_ids)) {
                throw new Exception("No recipients found for the selected roles.");
            }

            // 3. Insert into `message_recipients` for each user
            ...
            mysqli_commit($link);
            echo json_encode(['success' => true, 'message' => 'Message sent successfully to ' . count($recipient_ids) . ' users.']);
